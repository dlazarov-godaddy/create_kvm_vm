...
# Create VM
virt-install --name="${NAME}"  --vcpus="${CPUS}"  --memory="${MEMORY}" --location="${OS_IMAGE_PATH}"/"${OS_IMAGE}"  --disk path=/dev/mapper/"${vg}"-"${NAME}",size="${DISK}",bus=virtio,format=qcow2, --network=bridge="${BRIDGE}",model=virtio --extra-args='console=ttyS0,115200n8 serial'  --nographics
